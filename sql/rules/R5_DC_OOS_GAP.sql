...
req AS (
    ...
    WHERE l.ELIG_FLAG = 1
      AND l.ELIG_REASON = 'OK'
      AND COALESCE(l.MJ_REQ_NO_EXC, l.MJ_REQ, 0) > 0
      AND l.RDC IS NOT NULL
    GROUP BY l.SESSION_ID, l.WERKS, l.RDC, l.MAJ_CAT, l.GEN_ART_NUMBER
),
-- DC stock per gen_art per RDC (sum across variants/sizes)
dc_stock AS (
    SELECT
        d.RDC,
        d.GEN_ART_NUMBER,
        SUM(COALESCE(d.STK_QTY, 0))   AS RDC_STK_QTY,
        SUM(COALESCE(d.HOLD_QTY, 0))  AS RDC_HOLD_QTY,
        COUNT(DISTINCT d.ARTICLE_NUMBER) AS VAR_COUNT_AT_RDC
...
net_stock AS (
    SELECT
        d.GEN_ART_NUMBER,
        SUM(COALESCE(d.STK_QTY, 0)) AS NET_STK_QTY,
        COUNT(DISTINCT CASE WHEN COALESCE(d.STK_QTY, 0) > 0 THEN d.RDC END) AS RDC_WITH_STK
    FROM V2RETAIL.ARS_GOLD.V_SILVER_DC_STOCK d
